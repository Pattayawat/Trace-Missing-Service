...
resource "aws_apigatewayv2_route" "report_routes" {
  for_each = toset(["POST /reports", "GET /reports", "GET /reports/{id}", "PATCH /reports/{id}", "GET /incidents", "POST /reports/upload-url", "GET /reunifications", "PATCH /reports/{id}/status", "GET /reports/{id}/detail", "GET /stats"])
  api_id    = aws_apigatewayv2_api.main.id
  route_key = each.key
