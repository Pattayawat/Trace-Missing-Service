...
export function StatusCharts({ locations, persons }: StatusChartsProps) {
  ...
  const statusData = [
    {
      name: "คนหาย",
      value: persons.filter((p) => ["REPORTED", "VERIFYING", "ACTIVE", "MATCHING"].includes(p.status)).length,
      fill: "var(--color-destructive)",
    },
    {
      name: "พบแล้ว",
      value: persons.filter((p) => ["VERIFIED", "REUNITED", "CLOSED"].includes(p.status)).length,
      fill: "var(--color-primary)",
    },
    {
      name: "ปลอดภัย",
      value: persons.filter((p) => (p.status === "ACTIVE" || p.status === "REPORTED") && p.type === "survivor").length,
      fill: "var(--color-success)",
    },
    {
      name: "ไม่ทราบตัวตน",
      value: persons.filter((p) => (p.status === "ACTIVE" || p.status === "REPORTED") && p.type === "unidentified-body").length,
